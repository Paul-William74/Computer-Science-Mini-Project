...
public class ImageProcessor {
    ...
    public AdjacencyListGraph<Node, Double> buildNodeGraph(File imageFile) throws IOException {
        ...
        lastBifurcationCount = 0;

        for (Junction j : raw.values()) {
            if (j.degree >= 3) {
                lastBifurcationCount++;
            }
        }

        lastBifurcationCount = Math.max(1, lastBifurcationCount / 8);

        System.out.println("Detected raw nodes = " + raw.size());
        System.out.println("Merged nodes = " + nodes.size());
        System.out.println("Estimated bifurcation count = " + lastBifurcationCount);
    ...
    private void createOutputFolderIfNeeded() {
        File folder = new File("src/Datasets/output");
    ...
    private int computeThreshold(int[][] img, boolean[][] mask) {
        ...
        return count == 0 ? 120 : (int) (sum / count) + 18;
    }

    private int[][] toBinary(int[][] img, int threshold, boolean[][] mask) {
        int w = img.length;
        int h = img[0].length;
    ...
    private Map<String, Junction> detectJunctions(int[][] skel) {
        ...
        for (int x = 1; x < skel.length - 1; x++) {
            for (int y = 1; y < skel[0].length - 1; y++) {
                ...
                if (n == 1 || n >= 3) {
                    map.put(x + "," + y, new Junction(x, y, n));
                }
            }
        }
    ...
    private List<Node> clusterJunctions(Map<String, Junction> raw, int[][] skel) {
        ...
        for (Node start : nodes) {
            for (int dx = -1; dx <= 1; dx++) {
                for (int dy = -1; dy <= 1; dy++) {
                    ...
                    follow(graph, map, nodes, start, nx, ny, skel);
                }
            }
        }
    }
    ...
    private int[] nextPixel(int[][] skel, int px, int py, int cx, int cy) {
        for (int dx = -1; dx <= 1; dx++) {
            for (int dy = -1; dy <= 1; dy++) {
                if (dx == 0 && dy == 0) {
                    continue;
    ...
    private Node findNode(List<Node> nodes, int x, int y, Node ignore) {
        for (Node n : nodes) {
            if (n == ignore) {
                continue;
            }
    ...
    private void removeIsolated(AdjacencyListGraph<Node, Double> graph) {
